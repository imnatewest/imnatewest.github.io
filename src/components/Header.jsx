  ...
  return (
    ...
    >
      <path
        ...
        strokeLinejoin="round"
        strokeWidth={1.6}
        d="M12 3v2.25M12 18.75V21M5.636 5.636l1.59 1.59M16.773 16.773l1.59 1.591M3 12h2.25M18.75 12H21M5.636 18.364l1.59-1.591M16.773 7.227l1.59-1.59"
      />
...
function Header({ heroName, contactEmail, theme, onToggleTheme }) {
  const profileImage = useMemo(
    () => `${import.meta.env.BASE_URL}profilepic.jpeg`,
    []
  );
  const extractionGameHref = useMemo(
    () => `${import.meta.env.BASE_URL}extraction-game/index.html`,
    []
  );
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/50 bg-white/80 backdrop-blur transition-colors dark:border-slate-700/60 dark:bg-night/70">
      <div
        className={`${containerClass} flex items-center justify-between gap-4 py-4`}
      >
        <a
          href="#hero"
        ...
        <nav
          className="hidden items-center gap-6 text-sm font-medium text-mist md:flex dark:text-nightMuted"
          aria-label="Primary"
        >
          {navLinks.map((link) => (
            <a
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-slate-200/60 bg-white/95 px-4 py-4 shadow-lg transition dark:border-slate-700/60 dark:bg-nightSurface">
          <nav className="space-y-2" aria-label="Mobile primary">
            ...
            <a
              className={`${linkClasses} bg-ink text-white hover:bg-slate-900 dark:bg-white dark:text-night dark:hover:bg-slate-200`}
              href={extractionGameHref}
              onClick={closeMenu}
            >
              Extraction Game
